...
| Agent — local | **browser-only** (real GPU calls) / **T** (VRAM budget's scaling+backoff logic, measurement loop's control flow) | WebLLM manager (Qwen2.5-Coder 1.5B/3B/7B), install/activate, VRAM gate, streaming chat. Untestable in sandbox (no GPU) beyond the parts factored out and headless-tested against mocks. **VRAM budget tool:** local WebLLM's real failure mode is a full chat history resent every turn against a small, fixed 8192-token window — hit within roughly a dozen messages on real testing, not "long conversations." Rather than cap history (declined by design choice, may revisit), `measureVram()` empirically fills GPU buffers (`pushErrorScope('out-of-memory')`/`popErrorScope()` — WebGPU has no live free-VRAM query, verified against `@webgpu/types`) until one fails, reports the total, and lets the allocation be split with a portion given to Qwen. `agentPickContextWindow()` scales `context_window_size` from that allocation (confirmed via WebLLM's source that the KV cache does scale with this value at reload, not a fixed pre-compiled ceiling) with probe/retry — no exact MB-to-tokens formula exists, so each attempt gets a fresh engine (a failed reload can take the GPU device down with it per WebLLM's own `DeviceLostError`), backing off and — if every scaled attempt fails — **throwing an honest re-measure error; there is no flat fallback**, and no measurement means Qwen cannot be activated at all. The scaling anchor is `VRAM_FIGURES_CTX=4096` (dependency-verified: WebLLM's published `vram_required_MB` figures are all computed at a 4096-token window). The measurement persists to localStorage with a timestamp (its scope — origin+browser+machine — is exactly the snapshot's validity domain; the readout shows its age). The allocation slider's zones are two-ended (`vramZone`): red below the tier's `vramMB`, yellow to 2× it, green, then yellow/red at the top on **absolute** 512/256 MB reserves (not % — what they protect costs fixed MB); collision banners when a tier has no safe position; a "?" table shows per-model Min cache / Min VRAM. A snapshot, not a guarantee: VRAM is shared system-wide and can go stale between the test and actually loading the model — hence the tip to measure while the game runs. |
| Agent — Sight | **T** (assembly) / **browser-only** (e2e) | `buildContext()`: full current `main.lua` + conf summary + full Console since last Run/clear — **identical for both backends, no parameters, no gates** (the 14-line tail, the `AGENT_SIGHT_BUDGET`/`luaDigest` source gate, and the `chars/4` estimator are all deleted per the no-arbitrary-limits decision; chat history was never capped). Local overflow fails as a clean `ContextWindowSizeExceededError`, surfaced in chat with WebLLM's own exact prompt-vs-window counts + the remedy (`agentFailText` — raise the VRAM allocation, or Clear the chat). Assembly + overflow text tested headless against the extracted functions; per-turn freshness verified at wire level (two-turn simulation, real extracted send pipeline). **Open (browser-only):** user-reported mid-chat staleness — the agent seems not to see later edits; pipeline provably fresh headless, so it's model anchoring or a real editor→doc sync failure. Discriminating diagnostics in `TODO.md`'s "Sight freshness" backlog row. |
