...
|---|---|---|---|---|---|
| High | Medium | Backends | Backend interface + Remote | Local WebLLM (1.5B–7B) is free/private but too weak to carry Sight-driven reasoning or scaffolding discipline. Everything below this row needs a genuinely capable model to even be testable — the wall we hit this session. | `backend.chat(messages, {tools, stream})` interface — the one piece still missing. The rest is done (Settings → Agentic Coding — see Already built): per-agent key entry, and Commit fetches that provider's live `/models` list for real (Anthropic/OpenAI/xAI, with loading/error states) — no hardcoded or placeholder model IDs left. What's not wired is actually *chatting* with the toggled-active agent; `agentSend()` still only knows about the local WebLLM engine. Mistral, Meta Llama, and Gemini are deferred (not yet testable) but already confirmed CORS-reachable — see the Backlog row. |
