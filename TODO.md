...
- **Chat is wired to the toggled-active agent(s), group-chat style.** `agent.messages` carries a `from` field for attribution. Two independent toggles (local VRAM-active, one remote agent active) route 0/1/2 agents: 0 hides the chat tab; 1 is a plain two-way conversation with whichever is on; 2 pipelines local → remote — the local agent's "prompt engineering + context curation" step is a deliberate no-op for now (forward the user's raw message + full Sight, unmodified), marked by a code-generated `sys`-styled transcript line ("Qwen → Claude") rather than any fabricated dialogue, then the remote's real reply is what's shown. Remote calls (`remoteChat()`) hit Anthropic's `/v1/messages` and OpenAI/xAI's OpenAI-compatible `/v1/chat/completions` directly from the browser, non-streaming for this first pass; response bodies are surfaced on failure, matching the model-list fetch's error handling. Routing logic tested headless against the real extracted functions (0/1/2-agent branches, error surfacing) — the live network calls themselves are still browser-only per the claim ladder. Still Sight-only, no Hands.
- **Chat replies render properly, on the app's side, not the agent's.** Chat bubbles picked up the same `.out-md` markdown styling cell-output already had (they'd been missing it — the real cause of an unwrapped-code-forces-horizontal-scroll bug). Every fenced code block gets Lua syntax highlighting (`highlightLua()`, a small standalone regex tokenizer — not CodeMirror, since a real editor instance being torn down and rebuilt on every streamed token would be expensive and janky) plus a hover-reveal copy button, regardless of whether the agent tagged the fence as `lua` or not (this is a Lua-only IDE, and the offline markdown fallback drops fence-language tags anyway, so language-agnostic-and-always-Lua is the only default that works either way). Headless-tested against real chat content.
